...
function fn_init_currency($params, $area = AREA)
{
    ...
    $currencies = db_get_hash_array("SELECT a.*, b.description FROM ?:currencies as a LEFT JOIN ?:currency_descriptions as b ON a.currency_code = b.currency_code AND lang_code = ?s $join WHERE 1 ?p $order_by", 'currency_code', CART_LANGUAGE, $cond);
...
function fn_init_layout($params)
{
    ...
    $key_name = 'stored_layout' . (Registry::get('runtime.embedded') ? '_embedded' : '');
    ...
    fn_set_session_data($key_name, $layout['layout_id']);
