...
* @param bool $embedded - if set to true, edirects to host URL, attaching our URL after hash
* @return
*/
...
EOU;
    ...
    // FIXME: workaround
    if (defined('STORE_IMPORT')) {
        $msg = is_array($error) ? implode("\n", $error) : $error;
        if ($debug_data) {
            $msg .= print_r($debug_data, 1);
        }
 ...
 * @return boolean always true
 */
function fn_install_theme($layout_id, $theme_name, $company_id = null, $install_default_scheme = true)
{
    // Copy files
    ...
    // Import theme layout
    if ($install_default_scheme && file_exists($repo_dest . '/layouts.xml')) {
...
function fn_delete_theme($theme_name)
{
    ...
    foreach ($layouts as $layout) {
        if ($layout['is_default']) {
            fn_set_notification('E', __('error'), __('error_delete_theme_layout'));
            $can_delete = false;
            break;
        }
    }
...
function fn_merge_styles($files, $styles='', $prepend_prefix = '', $params = array())
{
    ...
    arsort($names);
    $hash = md5(implode(',', $names) . PRODUCT_VERSION) . fn_get_storage_data('cache_id');
...
function fn_get_preview_url($uri, $object_data, $user_id)
{
    ...
    if ($object_data['status'] != 'A') {
        $uri = fn_link_attach($uri, 'action=preview');
        $uri = urlencode($uri);

        $preview_url = fn_url("profiles.act_as_user?user_id=$user_id&area=C&redirect_url=$uri", 'A');
    } else {
