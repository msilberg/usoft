...
class General
{
    ...
    {
        return db_connect_to(array('dbc_name' => General::CONNECTION_NAME), $store_data['db_name']);
    }
    ...
    {
        return db_connect_to($params, Registry::get('config.db_name'));
    }
    ...
    public static function processAddons($store_data, $class_name, $addons = array())
    {
        ...
        self::setEmptyProgressBar('store_import.processing_addons');
        if (empty($addons)) {
            return true;
        }
    ...
    {
        return (!empty($param) && !empty(self::$supplier_settings[$param])) ? self::$supplier_settings[$param] : self::$supplier_settings;
    }
    ...
    public static function createLogo($company_id = 0, $layout_id = 0, $filename = '', $img_path = '', $type = 'theme', $common_descr_object = 'Customer_logo')
    {
        ...
        foreach (db_get_fields("SELECT lang_code FROM ?:languages") as $lang_code) {
            ...
            if (!empty($descr)) {
                db_query("INSERT INTO ?:common_descriptions (object_id, description, lang_code, object_holder) VALUES (?i, ?s, ?s, 'images')", $img_id, $descr, $lang_code);
            }
        }
    ...
    public static function convertPresets()
    {
        $theme_name = 'basic';
        $schema_path = fn_get_theme_path('[themes]/' . $theme_name . '/presets/schema.json', 'C');
        ...
        foreach ($presets as $preset) {
                ...
                $preset_path = fn_get_theme_path('[themes]/' . $theme_name . '/presets/data/', 'C');
                    ...
                    foreach ($fields as $field_id => $value) {
                        ...
                        case 'backgrounds':
                            ...
                            foreach ($value as $bg_name => $bg_value) {
                                ...
                                case 'image_data':
                                    $less[$schema[$section]['fields'][$field_id]['properties']['image']] = !empty($bg_value) ? 'url("' . $bg_value . '")' : 'transparent';
                                    break;
            ...
            file_put_contents(fn_get_theme_path('[themes]/' . $theme_name . '/presets/data/' . $preset['name'] . '.less', 'C'), $less);
        }
    ...
    {
        if ($store_data['product_version'] == '4.0.1') {
            return false;
        }
        $mapping = array(
            '4' => array(
    ...
    public static function setActualLangValues()
    {
        ...
        db_query("INSERT INTO ?:language_values (SELECT ?s as lang_code, name, value, original_value FROM " . $prefix . "language_values WHERE lang_code = ?s) ON DUPLICATE KEY UPDATE ?:language_values.value = ?:language_values.value", General::$default_language, DEFAULT_LANGUAGE);
        if (fn_allowed_for('ULTIMATE')) {
