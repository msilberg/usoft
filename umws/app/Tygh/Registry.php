...
class Registry
{
    ...
    public static function set($key, $value, $no_cache = false)
    {
        ...
        if ($no_cache == false && isset(self::$_cached_keys[$_key])) {
            // if key should be cached - check if it was changed
            while (!self::_acquireLock($_key, self::$_cached_keys[$_key]['cache_level'])) {
    ...
    private static function _saveCache($key, $val)
    {
        ...
        self::$_cache->releaseLock($key, self::$_cached_keys[$key]['cache_level']);
    }
