...
class Languages
{
    ...
    */
     ...
     */
    public static function getLangPacksMeta($base_path = '', $pack_file = '', $reinstall = false)
    {
            ...
            if ($lc !== false) {
                ...
                foreach ($lang_data as $var_name => $var_data) {
                    if (!empty($var_name)) {
                        ...
                        $original_value = empty($var_data['ccomment']) ? '' : $var_data['ccomment'];
                ...
                foreach ($setting_options as $var_name => $var_data) {
                    if (!empty($var_name)) {
                        ...
                        $object_id = db_get_field('SELECT object_id FROM ?:settings_objects WHERE name = ?s', $var_name);

                        $query[] = db_quote('(?i, ?s, ?s, ?s, ?s)', $object_id, 'O', $lang_meta['lang_code'], trim($value), trim($original_value));
                    }
                ...
                foreach ($setting_variants as $var_name => $var_data) {
                    if (!empty($var_name)) {
                        ...
                        $variant_id = db_get_field('SELECT variant_id FROM ?:settings_variants WHERE name = ?s', $var_name);
