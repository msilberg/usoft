    ...
    public function __construct($request)
    {
        ...
        if (!empty($_SERVER['HTTP_REFERER'])) {
            ...
            if ($origin_host != Registry::get('config.current_host')) { // cross-domain request
                Registry::set('runtime.embedded', true);

                if (!empty($request['init_context'])) {
                   $_SESSION['init_context'] = $request['init_context'];
                }

                if (!empty($_SESSION['init_context'])) {
                    $this->_init_context = $_SESSION['init_context'];
                }
    ...
    public function __destruct()
    {
        ...
        if ($called == false) {
            ...
            if (!empty($this->result_ids)) {
                ...
                if (Registry::get('runtime.embedded') && empty($this->_result['html']) && $this->_skip_result_ids_check == false && !empty($text)) {
                    foreach ($this->result_ids as $r_id) {
                        $text .= '<script type="text/javascript">document.process.submit();</script>';
                        $this->assignHtml($r_id, $text);
                        break;
                    }
                }
            ...
            if ($this->_request_type == self::REQUEST_XML) {
                ...
                $response = '<script type="text/javascript" src="' . Registry::get('config.current_location') . '/js/lib/jquery/jquery.min.js' . '"></script>
                             ...
                             <script type="text/javascript">Tygh.$.postMessage(
                                "' . fn_js_escape($response) . '",\'' . $this->_init_context . '\');</script>';
