...
<div class="minisite minisite-<?php print $wdir ?>">
	...
	<?php
		...
		{
			print "<div class=\"storemap-btn sm-btn sm-btn-pass\" style=\"top:335px;\"><div>";
			data::lbl_otp(83);
			print "</div></div>";
		}
		/*if (service::mbtns_check(3))
			print "<div id=\"".(++$no)."\" class=\"contact-btn sm-btn sm-btn-pass\" style=\"top:".(($i--)*$vars['mlt']-$vars['fct'])."px;\"><div>".db::lbl(91)."</div></div>";
		if (service::mbtns_check(2))
			print "<div id=\"".(++$no)."\" class=\"gdisc-btn sm-btn sm-btn-pass\" style=\"top:".(($i--)*$vars['mlt']-$vars['fct'])."px;\"><div>".db::lbl(82)."</div></div>";*/
		...
		if (service::mbtns_check(6))
			print "<div id=\"".(++$no)."\" class=\"price-btn sm-btn sm-btn-pass\" style=\"top:".(($i--)*$vars['mlt']-$vars['fct'])."px;\"><div>".db::lbl(125)."</div></div>";
		if (service::mbtns_check(0))
			print "<div id=\"".(++$no)."\" class=\"video-btn sm-btn sm-btn-pass\" style=\"top:".(($i--)*$vars['mlt']-$vars['fct'])."px;\"><div>".db::lbl(80)."</div></div>";	
	?>
	</div>
</div>