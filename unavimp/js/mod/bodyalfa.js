...
uServe.toggleListBtn = function(switcher){
	if (switcher && !($("div.show-st-list").is(":visible"))){ $("div.show-st-list").show() }
		...
		$("div.ls-cont").load(addrUrl.loadBlock("slider"), function(){
			$.getJSON(addrUrl.vapi + "slides_num,gs_num", function(data){
				...
				switch(bVars.slType){
					...
					break;
					case 2:
						$("div.back2scat").mousedown(function(){ uBody.scatsWall(bVars.cat) });
...
uBody.closePlayer = function(){
	...
	if ($("div.videos-matrix").length != 0){ $("div.videos-matrix").fadeIn(800) }
}
uBody.openPlayer = function(trailerId,singleTrailer){
	var videoTBase = 300; // supremum of the entire video block
	var videoSBase = 315; // side alignment of the entire video block
	var clsTCorr = 30;
	var clsSCorr = 30;
	var videoDir,sideCh,topPos,sidePos;
	if ($("div.videos-matrix").length != 0){ $("div.videos-matrix").hide() }
	$.getJSON(addrUrl.api + 14, function(data){
		videoDir = addrUrl.base + "stores/" + data['tc'] + "/" + data['store'] + "/videos/";
		$("div.minisite-cont").append("<div class='video-player'><div id='videoplayer7267'></div></div>");
	...
	$.get(addrUrl.api + 13, function(data){
		if (parseFloat(data) > 1){
			$("div.video-layer").load(addrUrl.loadBlock("vtrailers"), function(){
				$("div.vtrailer-icon").mousedown(function(){ uBody.openPlayer($(this).attr("id")) })
			});
...
uBody.slTriangle = function(switcher){
	var iNum,startPoint;
	if (switcher){ iNum = 9 }else{ iNum = (bVars.divcbtnNo - 9) }
...
uBody.goFullscreen = function(){
	...
	$("div#scl-cnt").css({"top":"15px","left":"30px","z-index":"16"});
	bMap.map.updateSize();
}
...
$(document).ready(function(){
	uServe.setBrwsPref();
	$.getJSON(addrUrl.vapi + "lang,wdir", function(data){
		bMap.varsArr = data;
	...
	$("div.sibtn").mousedown(function(){ uBody.openMsgBox(124) });
